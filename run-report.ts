...
async function main() {
  ...
  for (const proj of projections) {
    try {
      ...
      const normalizedTeam = normalizePPTeamAbbrev(proj.team);
      ...
      const opponent = game.homeTeam === normalizedTeam ? game.awayTeam : game.homeTeam;
      const homeAway = game.homeTeam === normalizedTeam ? 'home' : 'away';
      ...
      const matchup = await analyzeMatchup(
        ...
        homeAway,
        game.spread,
        normalizedTeam // normalized abbrev for pace/B2B lookup
      );
  ...
  let report = `# 🏀 PrizePicks Daily Report — ${today}\n\n`;
  ...
  const outPath = `/Users/jeffnwagbo/clawd/memory/prizepicks-report-${dateStr}.md`;
